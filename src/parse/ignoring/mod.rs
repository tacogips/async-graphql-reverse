    ...
    ( $component:ident,
      ...
      $inerface_names_set:ident
            ...
            .element_value_type_def(&$structured_schema.definitions)
        {
            Err(e) => {
                println!(
                    "WARN: elemnt value type erro {:?}, error {}",
                    $each_field.typ, e
                );
            }
            Ok(typ) => {
...
fn inner_remove_ignored_from_structure(
    ...
    let mut enum_names_set: HashSet<&String> = HashSet::new();
    let mut object_names_set: HashSet<&String> = HashSet::new();
    let mut input_object_names_set: HashSet<&String> = HashSet::new();
    let mut union_names_set: HashSet<&String> = HashSet::new();
    let mut inerface_names_set: HashSet<&String> = HashSet::new();
    let mut scalar_names_set: HashSet<&String> = HashSet::new();

    if let Some(ignore_enums) = &ignore.r#enum {
        structured_schema
            .definitions
            .enums
            .retain(|name, _| !ignore_enums.contains(name));
        enum_names_set = ignore_enums.iter().collect();
    }
    ...
    if let Some(ignore_objects) = &ignore.object {
        structured_schema
            .definitions
            .objects
            .retain(|name, _| !ignore_objects.contains(name));

        object_names_set = ignore_objects.iter().collect();
    }
    ...
    if let Some(ignore_input_object) = &ignore.input_object {
        structured_schema
            .definitions
            .input_objects
            .retain(|name, _| !ignore_input_object.contains(name));

        input_object_names_set = ignore_input_object.iter().collect();
    }
    ...
    if let Some(ignore_union) = &ignore.union {
        structured_schema
            .definitions
            .unions
            .retain(|name, _| !ignore_union.contains(name));

        union_names_set = ignore_union.iter().collect();
    }
    ...
    if let Some(ignore_intrerface) = &ignore.interface {
        structured_schema
            .definitions
            .interfaces
            .retain(|name, _| !ignore_intrerface.contains(name));

        inerface_names_set = ignore_intrerface.iter().collect();
    }
    ...
    if let Some(ignore_scalar) = &ignore.scalar {
        structured_schema
            .definitions
            .scalars
            .retain(|name, _| !ignore_scalar.contains(name));

        scalar_names_set = ignore_scalar.iter().collect();
    }
